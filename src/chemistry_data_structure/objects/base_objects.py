...
class _2DChemicalObj:
    ...
    def heavy_atoms(self):
        return list(
            atom.get_index() for atom in self.atom_objects if atom.element != "H"
        )
    ...
    def non_bonded_electrons(self):
        return {
            atom_id: self.get_atom(atom_id).non_bonded_electrons
    ...
    def chirality(self):
        return {
            atom_id: self.get_atom(atom_id).chirality
            for atom_id in self.atoms
    ...
    def neighbour_counts(self):
        return {
            atom_id: len([x for x in self.graph.neighbors(atom_id)])
            for atom_id in self.atoms
        }
    ...
    def first_neighbours(self):
        return {
            atom_id: [x for x in self.graph.neighbors(atom_id)]
            for atom_id in self.atoms
        }
    ...
    def add_atom(self, atom: Atom2D) -> None:
        if not isinstance(atom, Atom2D):
            ...
            raise TypeError(
                f"atom must be of type Atom2D, but is of type f{type(atom)}"
            )
    ...
    def rename_atom(self, atom: Atom2D, name):
        nx.relabel_nodes(self._graph, {atom.name: name}, copy=False)
    ...
    def get_atom(self, index: Any, index_type="name") -> Atom2D:
        if index_type == "name":
            ...
            match_list = [
                a for a in self._graph._node.values() if a.index[index_type] == index
            ]
    ...
    def get_atoms(self, index: List, index_type="name"):
        ...
        if index_type == "name":
            ...
            return [
                [a for a in self._graph._node.values() if a.index[index_type] == i][0]
                for i in index
            ]
    ...
    def get_heavy_atom_bonds(self):
        ...
        return [
            ...
            if self.get_atom(bond[0]).element != "H"
            and self.get_atom(bond[1]).element != "H"
        ]
    ...
    def get_bond_orders(self, index: Iterable[Tuple[str]]):
        ...
        """
        return {
            frozenset((a1_id, a2_id)): self.bond_orders[frozenset((a1_id, a2_id))]
            for a1_id, a2_id in index
        }
    ...
    def get_neighbour_element_counts(self, element: str):
        ...
        for atom_id in self.atoms:
            counts[atom_id] = len(
                [
                    n_id
                    for n_id in self.graph.neighbors(atom_id)
    ...
    def draw_graph(
        ...
        """
        node_font_sz, edge_font_sz, charge_font_sz = (
            font_sizes["node"] if font_sizes else node_size // 50,
        ...
        if backbone_only:
            ...
            formal_charges = {
                a.get_index(): a.formal_charge for a in graph.nodes.values()
            }
            chirality = {a.get_index(): a.chirality for a in graph.nodes.values()}
            bond_orders = {
                frozenset(bond_ids): self.get_bond(bond_ids[0], bond_ids[1]).order
                for bond_ids in graph.edges()
            }
            ...
            for a_id, a in self.atoms.items():
                if a.element == "H" and set(first_neighbours[a_id]).issubset(
                    marked_atom_ids
                ):
        ...
        for atom in graph.nodes.values():
            ...
            if draw_marked_atoms:
                ...
                colour_map.append(
                    ELEMENT_COLOURS[atom.element.strip("0123456789").upper()]
                )
        ...
        if fixed_heavy_atoms is not None:
            pos = nx.spring_layout(
                graph, pos=fixed_heavy_atoms, fixed=fixed_heavy_atoms.keys()
            )
        else:
            kamada_kawai = True
    ...
    def assign_bond_orders_and_charges_with_ILP(
        ...
        assert not (
            allow_radicals and enforce_octet_rule
        ), "Can't simultaneously allow_radicals and enforce octet rule."

        problem = LpProblem(
            "Lewis problem (bond order and charge assignment)", LpMinimize
        )
        ...
        charges = {
            atom.get_index(): LpVariable(
                "C_{i}".format(i=atom.get_index()),
                -MAX_ABSOLUTE_CHARGE,
                MAX_ABSOLUTE_CHARGE,
                LpInteger,
            )
        ...
        non_bonded_electrons = {
            atom_id: LpVariable(
                "N_{i}".format(i=atom_id),
                0,
                MAX_NONBONDED_ELECTRONS // ELECTRON_MULTIPLIER,
                LpInteger,
            )
        ...
        bond_mapping = {bond: i for (i, bond) in enumerate(self.bonds)}
        ...
        OBJECTIVES = [
            ...
            sum(
                [
                    charge * ELECTRONEGATIVITIES[self.get_atom(atom_id).element.upper()]
            ...
            [
                sum(
                    [
                        bond_order
                        * ELECTRONEGATIVITIES[self.get_atom(atom_id).element.upper()]
        ...
        if total_electrons is not None:
            problem += (
                ELECTRONS_PER_BOND * sum(bond_orders.values())
                + ELECTRON_MULTIPLIER * sum(non_bonded_electrons.values())
                == total_electrons,
                "Known total electrons",
            )

        for atom in self.atoms.values():
            problem += charges[atom.get_index()] == VALENCE_ELECTRONS[
                atom.element.upper()
            ] - sum(
                [bond_orders[bond] for bond in self.bonds if atom.get_index() in bond]
            ) - ELECTRON_MULTIPLIER * non_bonded_electrons[
                atom.get_index()
            ], "{element}_{index}".format(
                element=atom.element.upper(), index=atom.get_index()
            )

        # Deal with absolute values
        for atom in self.atoms.values():
            problem += charges[atom.get_index()] <= absolute_charges[
                atom.get_index()
            ], "Absolute charge contraint 1 {i}".format(i=atom.get_index())
            problem += -charges[atom.get_index()] <= absolute_charges[
                atom.get_index()
            ], "Absolute charge contraint 2 {i}".format(i=atom.get_index())
                    ...
                    problem += (
                        ...
                        * sum(
                            [
                                bond_orders[bond]
                                for bond in self.bonds
                                if atom.get_index() in bond
                            ]
                        )
                        + ELECTRON_MULTIPLIER * non_bonded_electrons[atom.get_index()]
                        == (2 if atom.element.upper() in {"H", "HE"} else 8),
        ...
        try:
            ...
            args_id = ",".join(
                map(str, [enforce_octet_rule, allow_radicals, bond_order_constraints])
            )
        ...
        write_to_debug(
            debug,
            "Objective function values: {0}".format(
                [value(objective) for objective in OBJECTIVES]
            ),
        )
        ...
        for v in problem.variables():
            ...
            if variable_type == "C":
                ...
                self.get_atom(variable_substr).non_bonded_electrons = (
                    round(v.varValue) * ELECTRON_MULTIPLIER
                )
                if (
                    allow_radicals
                    and self.get_atom(variable_substr).non_bonded_electrons % 2 == 1
                ):
    ...
    def assign_aromatic_bonds(self):
        ...
        ring_bonds = {ring: bonds_for_ring(ring) for ring in rings}
        ...
        def is_hucklel_compatible(bond_orders: List[int]) -> bool:
            ...
            """
            return (
                sum([2 for bond_order in bond_orders if bond_order == 2]) - 2
            ) % 4 == 0
        ...
        def is_bond_sequence_aromatic(bond_orders: List[int]) -> bool:
            ...
            """
            cyclic_bond_orders = (
                [bond_orders[-1]] + list(bond_orders) + [bond_orders[0]]
            )
            for pair_of_bond_orders in zip(
                cyclic_bond_orders, [cyclic_bond_orders[-1]] + cyclic_bond_orders[:-1]
            ):
        ...
        def is_aromatic_ring(ring: List[int]) -> bool:
            if len(ring) % 2 == 0:
                ...
                return is_bond_sequence_aromatic(
                    ring_bond_orders[ring]
                ) and is_hucklel_compatible(ring_bond_orders[ring])
            else:
                # For odd-membered rings, include sp2 (pi electrons) lone pairs, and ensure Huckel's rule is enforced.
                non_bonded_pairs = reduce(
                    lambda acc, e: acc + e,
                    [
                        [2 for _ in range(0, self.non_bonded_electrons[atom_id] // 2)]
                        for atom_id in ring
                        if is_sp2(atom_id)
                    ],
                    [],
        ...
        for ring in rings:
            if is_aromatic_ring(ring):
                ...
                bond_prop_dict = {
                    (a1, a2): {"order": AROMATIC_BOND_ORDER, "is_aromatic": True}
    ...
    def assign_hybridisations_and_valences(self):
        ...
        for atom in self.atom_objects:
            ...
            if max(FULL_VALENCES[atom.element.upper()]) > 1:
                atom.hybridisation = (
                    neighbour_counts[atom.get_index()]
                    + self.non_bonded_electrons[atom.get_index()] // 2
                    - 1
                )
            else:
    ...
    def assign_conjugated_atoms(self):
        ...
        for atom in self.atom_objects:
            ...
            if num_heavy_atom_neighbours > 1:
                ...
                hybridisation_sum = sum(
                    [self.hybridisations[atom.get_index()]]
                    + [
                        self.hybridisations[neighbour_id]
                        for neighbour_id in first_neighbours[atom.get_index()]
                        if max(
                            FULL_VALENCES[self.get_atom(neighbour_id).element.upper()]
                        )
                        > 1
                    ]
                )

                num_heavy_atoms_in_calc = (
                    len(
                        [
                            neighbour_id
                            for neighbour_id in first_neighbours[atom.get_index()]
                ...
                normalised_hybridisation = (
                    hybridisation_sum - self.non_bonded_electrons[atom.get_index()] // 2
                ) / num_heavy_atoms_in_calc

                hybridisation_scores[atom.get_index()] = normalised_hybridisation

                # Conjugation variable CJ = 1 if normalised_hybridisation <= 2.25
                self.get_atom(atom.get_index()).is_conjugated = (
                    1 if normalised_hybridisation <= 2.25 else 0
                )
    ...
    def get_fragments_around_elements(
        self, elements: set, max_depth: int, restrict_arom: bool = False
    ) -> graph:
        """
        ...
        for e in elements:
            ...
            for g_id in g_ids:
                ...
                while not search_queue.empty():
                    ...
                    h_ids = set(
                        n_id for n_id in n_ids if self.get_atom(n_id).element == "H"
                    )
...
class _3DChemicalObj(_2DChemicalObj):
    ...
    def atom_coord_matrix(self):
        return np.array([atom.coordinates for atom in self.atom_objects])
